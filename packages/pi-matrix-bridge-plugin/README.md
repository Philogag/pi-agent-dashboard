...
- **设置页**（Settings → Pi Matrix Bridge）三块面板：
  ...
  - 后台会话状态/日志 + 启动/停止/重启。
- **配置持久化**：经 `plugin_config_write` 写入 `~/.pi/dashboard/config.json`
  （`plugins.pi-matrix-bridge.*`），并镜像到 `~/.pi/matrix-bridge.json`（chmod 600，
  `trustedUsers` 加 `matrix:` 前缀），以便手动运行的桌面 pi 共用同一套连接/配对配置。
...
**覆盖** 文件 `~/.pi/matrix-bridge.json`。本插件直接以 spawn 子进程并把这两个 env 注入后台会话，
同时把配置镜像写入文件；两者保持一致。手动直接运行 `pi` 时则靠文件里的镜像配置生效。
